...
\dfn{IP level graph} $G_{ip}$, built by the IP addresses and links found trough
\traceroute. Second, the \dfn{router level graph} $G_{r}$, getting after solve
alias resolution process through MIDAR~\cite{Keys13}). Third, the \dfn{MPLS
router level graph} $G^{mpls}_{r}$, formed by MPLS links and routers in which at
...
The summary of the top ASes \ed{BD: Top ASes in terms of what?} is shown in
